...
assume mult1,
assume mult2,
cases mult1 with num1 pf1,
cases mult2 with num2 pf2,
...
example 
  ...
  (SomeoneKnowsLogic : ∃ (p), KnowsLogic p):
   ∃ pe : Person, isCool pe := 
begin
  cases SomeoneKnowsLogic with Eth Ethknowslogic,
  apply exists.intro Eth,
  have h := LogicMakesCool Eth,
  have EthCool := h Ethknowslogic,
  exact EthCool,
end
...
assume e,
assume nuh,
cases nuh with  perp np,
have hperp := e perp,
contradiction,

assume nuh,
assume per,
cases classical.em (P per),
exact h,

have f := nuh (exists.intro per h),
contradiction,
...
assume nowp,
assume o,
cases classical.em ( P o) with r nr,
have f := nowp (exists.intro o r),
contradiction,
...
cases pq with p q,
cases q with po qo,
have ex := exists.intro p po,
exact or.inl ex,


have exq := exists.intro p qo,
exact or.inr exq,
end
